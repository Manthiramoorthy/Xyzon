                        ...
                        <div className="d-flex align-items-center gap-2 ms-auto">
                            {user && !hasCustom && (
                                <button
                                    ...
                                    onClick={() => navigate(user.role === 'admin' ? '/admin/certificate' : '/user/certificate')}
                                >
            ...
            {hasCustom && (
                <div id="panel-side-menu" style={{ position: 'fixed', top: 64, left: 0, bottom: 0, width: 240, background: '#fff', borderRight: '1px solid #00006622', transform: open ? 'translateX(0)' : 'translateX(-110%)', transition: 'transform .35s', boxShadow: '2px 0 8px rgba(0,0,0,0.08)', zIndex: 1190 }}>
                    <ul style={{ listStyle: 'none', margin: 0, padding: '1rem .75rem', display: 'flex', flexDirection: 'column', gap: 4 }}>
                        {items.map(it => (
                            <li key={it.to}>
                                <Link onClick={close} to={it.to} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 12px', textDecoration: 'none', fontWeight: 600, borderRadius: 10, background: location.pathname === it.to ? '#f26b24' : '#00006608', color: location.pathname === it.to ? '#fff' : '#000066', fontSize: 14 }}>
